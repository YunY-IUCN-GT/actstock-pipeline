...
def get_etf_top_holdings(
    ...
    try:
        ...
        if as_of_date is not None:
            ...
            # Use latest date
            query += " AND as_of_date = (SELECT MAX(as_of_date) FROM analytics_09_etf_top_holdings WHERE etf_ticker = %(etf_ticker)s)"
...
def get_all_etf_holdings(
    ...
    try:
        ...
        if as_of_date is not None:
            ...
            query += " AND as_of_date = (SELECT MAX(as_of_date) FROM analytics_09_etf_top_holdings WHERE time_period = %(time_period)s)"
    ...
    try:
        query = """
            ...
            WHERE holding_ticker = %(holding_ticker)s
              ...
              AND as_of_date = (SELECT MAX(as_of_date) FROM analytics_09_etf_top_holdings WHERE time_period = %(time_period)s)
