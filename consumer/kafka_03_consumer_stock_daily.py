...
"""
Kafka Consumer - Stock Daily Data
Consumes stock daily data from Kafka and stores in PostgreSQL
...
LOG = setup_logger(__name__, "kafka-consumer-stock-daily.log")


class StockDailyDataConsumer:
    """Kafka consumer for stock daily OHLC data"""
    ...
    def _connect_kafka(self):
        ...
        try:
            self.consumer = KafkaConsumer(
                ...
                group_id=KAFKA_CONFIG["consumer_group_id"] + "-stock-daily",
    ...
    def _store_stock_data(self, message: Dict):
        """
        Store stock daily data in database
        
        Args:
            message: Message from Kafka containing stock daily data
        """
        try:
            ...
            row_data = {
                'ticker': ticker,
                'company_name': message.get('company_name', ticker),
            ...
            if self.db.execute_query(insert_query, row_data):
                LOG.debug("Stored %s: date=%s, close=%.2f, sector=%s",
            ...
            LOG.error("Error storing stock data: %s", exc, exc_info=True)
            return False
    
    def run(self):
        """Main consumer loop"""
        LOG.info("Starting Stock Daily Data Consumer")
        LOG.info("Listening to topic: %s", self.topic)
        LOG.info("Consumer group: %s", KAFKA_CONFIG["consumer_group_id"] + "-stock-daily")
        ...
        try:
            for message in self.consumer:
                ...
                try:
                    if self._store_stock_data(message.value):
                        processed += 1
                    else:
                        errors += 1
...
def main():
    """Main execution"""
    consumer = StockDailyDataConsumer()
    consumer.run()
